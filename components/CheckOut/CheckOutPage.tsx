...
export default function CheckOutPage() {
    ...
    const handleCheckOut = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        await fetch(process.env.NEXT_PUBLIC_API_URL + '/orders', {
            ...
            body: JSON.stringify({
                ...
                customerAddress: address + ', ' + wardName.name + ', ' + districtName.name + ', ' + provinceName.name,
    ...
    useEffect(() => {
        ...
        const getProvinces = async () => {
            setLoading(true);
            await fetch(process.env.NEXT_PUBLIC_VN_API_URL + 'p')
                .then((res) => res.json())
                .then((res) => {
                    setProvinces(res);
                    setLoading(false);
    ...
    useEffect(() => {
        const getDistricts = async () => {
            ...
            await fetch(process.env.NEXT_PUBLIC_VN_API_URL + 'd')
                .then((res) => res.json())
                .then((res) => {
                    setDistricts(res.filter((item: any) => item.province_code == province));
    ...
    useEffect(() => {
        const getWards = async () => {
            setLoadingWards(true);
            await fetch(process.env.NEXT_PUBLIC_VN_API_URL + 'w')
                .then((res) => res.json())
                .then((res) => {
                    setWards(res.filter((item: any) => item.district_code == district));
                    setLoadingWards(false);
                })
                .catch((err) => {
                    console.log(err);
                                ...
                                <div className='w-full flex justify-between'>
                                    ...
                                    >
                                        <option className='text-[#fff]' disabled >Chọn Tỉnh / Thành phố</option>
                                        {
                                            provinces?.map((item: any) =>
                                                <option className='text-white block py-2' key={item._id + 'province'} value={item.code} >{item.name}</option>
                                            )
                                        }
                                    </select>
                                    <select
                                    ...
                                    >
                                        ...
                                        {!loadingDistricts && districts?.map((item: any) =>
                                            <option className='text-white block py-2' key={item._id + 'district'} value={item.code} >{item.name}</option>
                                        )}
                                    </select>
                                </div>
                                <div className='w-full flex justify-between'>
                                    ...
                                    >
                                        ...
                                        {!loadingWards && wards?.map((item: any) =>
                                            <option className='text-white block py-2' key={item._id + 'ward'} value={item.code} >{item.name}</option>
                                        )}
                                    </select>
                                        ...
                                        <li key={item._id + 'product'} className='flex w-full h-[85px] py-1 mb-5 rounded-[7px] bg-white'>
                                            ...
                                            <div className='ml-2 flex-1 h-full flex flex-col'>
                                                <div className='text-left w-full h-[40px] mb-1'>
                                                    <span className='font-medium text-[15px] leading-5 line-clamp-1 overflow-hidden whitespace-pre-wrap'>
                                                        {item.name}
                                                    </span>
                                                </div>

                                                <div className='w-full flex justify-between items-center'>
                                                    <span className='text-[13px] text-[#929292]'>Kích thước: {item.size}</span>
                                        ...
                                        {userData?.vouchers?.map((item: any) => item.status !== 1 &&
                                            <div key={item._id} className='w-full flex items-center my-1 justify-between'>
                                                ...
                                                {voucher._id === item._id ? (
                                                    ...
                                                    <span onClick={(e) => {
                                                        if (totalPriceCart >= item.condition) {
                                                            setVoucher(item)
                                                        }
                                                        else {
                                                            alert(`Đơn phải tối thiểu ${new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(item?.condition)}`)
                                                        }
                                                    }} className='font-medium cursor-pointer text-[15px] text-light-yellow mr-2'>Chọn</span>
                                                )}
                                            </div>
                                        )}
                                    </div>
